...
(() => {
  ...
  amendments.forEach(a => {
    ...
    toggle.value = "old";
    toggle.innerHTML = "Show Current";
    toggle.addEventListener("click", showOld, false);
    tbar.appendChild(toggle);

    toggle = document.createElement("button");
    toggle.value = "new";
    toggle.innerHTML = "Show Future";
  ...
  if (darkCss) {
    ...
    darkCss.media =
      colorScheme === "auto" ? "(prefers-color-scheme: dark)" : "";
    const render = document.createElement("div");
    function createOption(option) {
      const checked = option === colorScheme;
      return `
        <label>
          <input name="color-scheme" type="radio" value="${option}" ${
        checked ? "checked" : ""
      }>
          <span>${option}</span>
        </label>
